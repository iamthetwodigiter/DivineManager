...
class AnalyticsService {
        ...
        case AnalyticsPeriod.custom:
          if (customRange != null) {
            ...
            for (int i = 0; i < daysDifference; i++) {
              ...
              final dayRevenue = dayOrders.fold(0.0, (sum, order) => sum + order.totalPrice);
              int dayItemsSold = 0;
              for (final order in dayOrders) {
        ...
        case AnalyticsPeriod.weekly:
          for (int i = 7; i >= 0; i--) {
            ...
            final weekOrders = allOrders.where((order) => 
              order.timestamp.isAfter(weekStart) && order.timestamp.isBefore(weekEnd)
            ).toList();
            
            final weekRevenue = weekOrders.fold(0.0, (sum, order) => sum + order.totalPrice);
            int weekItemsSold = 0;
        ...
        case AnalyticsPeriod.monthly:
          for (int i = 5; i >= 0; i--) {
            ...
            final monthOrders = allOrders.where((order) => 
              order.timestamp.isAfter(monthStart) && order.timestamp.isBefore(monthEnd)
            ).toList();
            
            final monthRevenue = monthOrders.fold(0.0, (sum, order) => sum + order.totalPrice);
        ...
        case AnalyticsPeriod.yearly:
          for (int i = 2; i >= 0; i--) {
            ...
            final yearOrders = allOrders.where((order) => 
              order.timestamp.isAfter(yearStart) && order.timestamp.isBefore(yearEnd)
            ).toList();
            
            final yearRevenue = yearOrders.fold(0.0, (sum, order) => sum + order.totalPrice);
            int yearItemsSold = 0;
  ...
  Future<List<OrderAnalytics>> getTopSellingItems() async {
    try {
      ...
      final sortedItems = itemStats.entries
          .map((entry) => OrderAnalytics(
                itemName: entry.key,
  ...
  Future<AnalyticsOverview> getOverview(AnalyticsPeriod period, [DateTimeRange? customRange]) async {
    try {
      ...
      final totalOrders = salesData.fold(0, (sum, data) => sum + data.orderCount);
      final totalItemsSold = salesData.fold(0, (sum, data) => sum + data.itemsSold);
      final averageOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0.0;
      final totalInventoryValue = await _inventoryService.getTotalInventoryValue();
      
      final revenueGrowth = salesData.length >= 2 && salesData.first.revenue > 0
          ? ((salesData.last.revenue - salesData.first.revenue) / salesData.first.revenue) * 100
          : 0.0;
      ...
      return AnalyticsOverview(
        ...
        topSellingItem: topItems.isNotEmpty ? topItems.first.itemName : 'No sales yet',
